...
def main():
    try:
        print("=" * 80)
        print("KOSPI 상위 100개 종목 데이터 수집 시작")
        print("수집 우선순위: pykrx > 네이버금융 > DART API")
        print("=" * 80)
        ...
        # 상위 100개 종목
        df_top100 = df_cap.sort_values(by='시가총액', ascending=False).head(100)
        ...
        for idx, ticker in enumerate(df_top100.index, 1):
            ...
            # (3) DART API 데이터 (영업이익, 이익잉여금, 현금)
            op, re, cash = get_dart_financials(dart, ticker, current_year)
