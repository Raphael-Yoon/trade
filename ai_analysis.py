...
def analyze_stock_data(file_path):
    ...
    try:
        ...
        아래는 최근 수집된 국내 주식 시장의 재무 데이터(상위 30개 종목)입니다. 여기에는 전년 동기 대비 성장성 지표들과 **최근 주요 공시(Disclosures)** 정보가 포함되어 있습니다.
        ...
        **주의**: 
        ...
        - **공시 정보 활용**: '최근공시' 컬럼에 있는 '공급계약', '유상증자', '소송', '영업정지' 등의 중요 이벤트가 있다면 이를 종목 분석 시 핵심 재료로 적극 반영해 주세요. (없으면 재무 중심 분석)
        ...
        1. **종합 추천 의견 (Top 5 Picks)**:
           ...
           - 현재 시장 트렌드와 데이터상의 수치(공시 포함)를 결합하여 왜 이 종목이 선정되었는지 논리적으로 설명해 주세요.
...
def analyze_portfolio(portfolio_data):
    ...
    try:
        ...
        for s in portfolio_data:
            ...
            stock_type = s.get('type', 'portfolio')
            type_label = "[보유]" if stock_type == 'portfolio' else "[관심]"
            
            data_str += f"- 종목: {type_label} {name}({s.get('code', '')})\n"

            # 기본 정보
            if stock_type == 'portfolio':
                sl_ratio = s.get('stop_loss_ratio', 0)
                data_str += f"  현재가: {s.get('current_price', 0):,}원, 평단가: {s.get('purchase_price', 0):,}원, 수익률: {s.get('profit_rate', 0)}%, 설정 손절비율: {sl_ratio}%\n"
            else:
                data_str += f"  현재가: {s.get('current_price', 0):,}원 (관심 종목)\n"
            ...
            # 손절 정보
            sl_ratio = s.get('stop_loss_ratio', 0)
            sl_diag = s.get('sl_diagnosis', {})
            data_str += f"  - 손절비율: {sl_ratio}%\n"
            if sl_diag.get('signal') == 'SELL':
                reasons = ', '.join(sl_diag.get('reasons', []))
                data_str += f"  - [위험] 손절 신호 발생: {reasons}\n"
        ...
        제공된 포트폴리오 데이터를 객관적이고 논리적으로 분석하여, 전문적인 분석 리포트를 작성해 주세요.
        
        데이터에는 **[보유]** 종목(실제 매수 stock)과 **[관심]** 종목(Watchlist)이 섞여 있습니다.

        [보유 종목 데이터]
        {data_str}

        **작성 가이드라인**:
        1. **종합 총평**: 현재 포트폴리오의 건강 상태 및 관심 종목군에 대한 통합적인 요약을 먼저 제시해 주세요.
        2. **분석 요약 표**: 각 종목의 [종목명, 구분(보유/관심), 투자의견, 목표가, 상태/수익률, 핵심 전략]을 포함하는 마크다운 표를 작성해 주세요.
        3. **종목별 상세 진단 (### 종목명 형식 필수)**:
           - **상태 구분**: 해당 종목이 '보유'인지 '관심'인지 명시해 주세요.
           - **결론 (투자의견)**: 
             - 보유 종목: [적극매수 / 보유 / 비중축소 / 매도] 중 하나 선택.
             - 관심 종목: [신규진입 추천 / 관망 / 제외] 중 하나 선택.
           - **상세 분석**: 수익률(보유 시), 전문가 목표가 대비 괴리율, 실적 성장성, 최근 수급 상황을 종합적으로 분석해 주세요.
           - **기술적 분석**: 이동평균선(MA5, MA20) 이격도와 52주 고점대비 위치를 바탕으로 진입/탈출 시점을 진단해 주세요.
           - **뉴스 및 심리 분석**: 최신 뉴스 심리를 반영해 주세요.
           - **대응 전략**: 
             - 보유 종목: 구체적인 익절/손절가 또는 비중 조절 전략.
             - 관심 종목: 적정 매수 타점(가격대) 또는 진입을 위한 전제 조건 제시.
        4. **시장 대응 제언**: 통합적인 관점에서의 리스크 및 기회 요인 조언.

        **톤앤매너**:
        - 전문적이면서도 객관적인 분석가의 어조로 작성해 주세요.
        - 수치를 적극 활용하고, 한국어로 작성해 주세요.
        """
